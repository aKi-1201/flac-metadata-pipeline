...
def main():
    if len(sys.argv) < 2:
        print("Usage: python extract_album.py <album_folder> [album_folder2] [album_folder3] ...")
        sys.exit(1)

    album_folders = sys.argv[1:]
    ...
    for folder in album_folders:
        if os.path.isdir(folder):
            ...
            print(f"WARNING: Album folder not found, skipped: {folder}", file=sys.stderr)

    if not valid_folders:
        print("ERROR: No valid album folder was provided.")
        sys.exit(1)
    ...
    for album_folder in valid_folders:
        for root, dirs, files in os.walk(album_folder):
            ...
            for name in files:
                if name.lower().endswith(".flac"):
                    ...
                    dedup_key = os.path.normcase(os.path.realpath(full_path))
